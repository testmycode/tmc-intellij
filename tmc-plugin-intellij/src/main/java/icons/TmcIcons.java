package icons;

import com.intellij.openapi.util.IconLoader;

import javax.swing.Icon;

public interface TmcIcons {
    Icon SUBMIT_BUTTON = IconLoader.getIcon("submit.png");
    Icon DONE_EXERCISE = IconLoader.getIcon("done.png");
    Icon NOT_DONE_EXERCISE = IconLoader.getIcon("notdone.png");
    Icon RUN_BUTTON = IconLoader.getIcon("runbutton.png");
    Icon TEST_BUTTON = IconLoader.getIcon("testbutton.png");
    Icon UNKNOWN = IconLoader.getIcon("unknown.png");
    Icon REFRESH = IconLoader.getIcon("refresh.png");
    Icon SUCCESS = IconLoader.getIcon("smiley.png");
    Icon SIDE_PANEL = IconLoader.getIcon("sidepanel_new.png");
}



